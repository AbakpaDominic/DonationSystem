...
class Cron implements CronInterface {
   ...
   *   The logger service.
   */
  public function __construct(EntityTypeManagerInterface $entity_type_manager, RecurringOrderManagerInterface $recurring_order_manager, TimeInterface $time, LoggerInterface $logger) {
  ...
  public function run() {
    ...
    /** @var \Drupal\advancedqueue\Entity\QueueInterface|null $recurring_queue */
    $recurring_queue = $queue_storage->load('commerce_recurring');

    if ($recurring_queue instanceof QueueInterface) {
      $this->enqueueOrders($recurring_queue);
      $this->enqueueSubscriptions($recurring_queue);
    } else {
      $this->logger->error('Recurring queue could not be loaded.');
    }
  }
